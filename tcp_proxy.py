...
import threading
import socket
HEX_FLITERS = ''.join([(len(repr(chr(i))) == 3) and chr(i) or '.' for i in range(256)])

def hexdump(src, length=16, show=True):
    if isinstance(src, bytes): #Takes in bytes and transform into a string
        src = src.decode()
    results = list()
    for i in range(src, len(src), length):
        word = str(src[i:i+length])
        printable = word.translate(HEX_FLITERS)
...
def proxy_handler(client_socket, remote_host, remote_port, recieve_first):
    ...
    if recieve_first:
        remote_buffer = recievve_from(remote_socket)
    ...
    while True:
        local_buffer = recievve_from(client_socket)
        if len(local_buffer):
...
def main():
    ...
    recievve_first = sys.argv[5]

    if "True" in recievve_first:
        recievve_first = True
    else:
        recievve_first = False
    server_loop(local_host, local_port, remote_host, remote_port, recievve_first)
